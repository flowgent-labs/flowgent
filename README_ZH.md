...
> **一款通用 AI Agents 编排引擎，精准定位于完全动态 Agent（如 Claude Code）与传统确定性工作流之间的最佳平衡点——既有 LLM 的动态智能，又具备 DAG 执行的可预测性、可追踪性和可靠性。**

Flowgent 是构建企业级 Agentic 应用的 **Layer 1** 基础设施。它将 LLM 驱动的 Agent 节点与确定性节点组合为单一 DAG，由拓扑调度器统一执行，并提供完整的状态机持久化——每个决策均可审计，每次运行均可恢复。
