...
// SkillPostgresStore wraps store.PostgresGenericStore[entities.SkillInfo].
type SkillPostgresStore struct {
	inner *store.PostgresGenericStore[entities.SkillInfo]
}

func NewSkillPostgresStore(pool *pgxpool.Pool) *SkillPostgresStore {
	return &SkillPostgresStore{
		inner: &store.PostgresGenericStore[entities.SkillInfo]{Pool: pool, Table: "llm_skill", IDCol: "name"},
	}
}
