...
// FlowRunSQLiteStore wraps storage.SQLiteGenericStore[entities.FlowRunInfo].
type FlowRunSQLiteStore struct {
	inner *storage.SQLiteGenericStore[entities.FlowRunInfo]
}
...
func (s *FlowRunSQLiteStore) Update(ctx context.Context, e *entities.FlowRunInfo) error {
	...
	scopeWhere, scopeArgs := s.inner.SqlScope(ctx).SQLiteWhere()
	args := append([]any{string(e.Status), vars, output, e.Error, e.StartedAt, e.FinishedAt, e.ID}, scopeArgs...)
	_, err = s.inner.Conn.ExecContext(ctx,
		`UPDATE orh_flowrun SET status=?1, vars=?2, output=?3, error=?4, started_at=?5, finished_at=?6, updated_at=CURRENT_TIMESTAMP WHERE id=?7 AND (`+scopeWhere+`)`,
		args...)
	return err
}
...
func (s *FlowRunSQLiteStore) Cancel(ctx context.Context, id string) error {
	...
	_, err := s.inner.Conn.ExecContext(ctx,
		`UPDATE orh_flowrun SET status='CANCELLED', updated_at=CURRENT_TIMESTAMP WHERE id=?1 AND (`+scopeWhere+`)`, args...)
	return err
}
