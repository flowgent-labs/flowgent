...
// ApprovalPostgresStore wraps storage.PostgresGenericStore[entities.ApprovalInfo].
type ApprovalPostgresStore struct {
	inner *storage.PostgresGenericStore[entities.ApprovalInfo]
}
...
func (s *ApprovalPostgresStore) UpdateApproval(ctx context.Context, e *entities.ApprovalInfo) error {
	scopeWhere, scopeArgs := s.inner.SqlScope(ctx).PostgresWhere(6)
	args := append([]any{e.Status, e.Approved, e.Comment, e.ResolvedAt, e.Token}, scopeArgs...)
	_, err := s.inner.Pool.Exec(ctx,
		`UPDATE human_approvals SET status=$1, approved=$2, comment=$3, resolved_at=$4, updated_at=NOW() WHERE token=$5 AND (`+scopeWhere+`)`,
		args...)
	return err
}
...
func (s *ApprovalPostgresStore) ListPending(ctx context.Context, namespace string) ([]*entities.ApprovalInfo, error) {
	...
	rows, err := s.inner.Pool.Query(ctx,
		fmt.Sprintf(`SELECT %s FROM human_approvals WHERE namespace_id=$1 AND status='PENDING' AND del_flag=false AND (%s) ORDER BY created_at DESC`, cols, scopeWhere), args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
