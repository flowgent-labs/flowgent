#!/usr/bin/env python3
"""Consecutive real-cluster UI E2E gate for security-autonomy-fixer.
...
NOTIFICATION_URL = (
    f"http://flowgent-e2e-webhook.{config.K8S_NAMESPACE}.svc.cluster.local:8080/notify"
)
...
def execute_round(round_number, build):
    ...
    forwards = runner._PortForwards(config.K8S_NAMESPACE).start()
    audit = None
    browser_ok = False
    run_id = ""
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Real UI system E2E consecutive-round gate")
    ...
    for round_number in range(1, args.rounds + 1):
        if not execute_round(round_number, build=round_number == 1 and not args.skip_first_build):
            print(
            ...
            return 1
        completed += 1
        print(f"\nPASS: {completed}/{args.rounds} consecutive real UI system rounds")

    (ROUND_REPORTS / "summary.json").write_text(
        json.dumps(
            {"result": "PASS", "consecutive_successes": completed, "required": args.rounds},
            indent=2,
        )
    )
