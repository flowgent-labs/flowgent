...
| 2026-08-09 | **FIXED**: s21/s23 verifier-created `test-flow-*` resources could leak into later scenarios and s11 only reported them as a warning. s21 now deletes setup flows in `finally`, s23 cleans up failed lifecycle flows, the deployer force-deletes application-mode pods/deployments during full redeploy, and s11 fails if any `test-flow-*` application pod/deployment survives after redeploy/import. |
