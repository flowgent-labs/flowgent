...
与 `examples/` 真实 E2E 的核心区别：此处外部 API 全部本地模拟，可零依赖 `go test` 运行（仅需 Docker PostgreSQL）；真实案例测试依赖外部真实 API（GitHub、SonarQube 等）。
...
```
tests/it/
  runner.go               ← 总入口：ITRunner harness（对应 examples/.../runner.py）
  ...
  engine/                  ← 各模块集成测试（对应 examples/.../scenarios/）
  apiserver/
  controller/
  notifier/
  knowledge/
