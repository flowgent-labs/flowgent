...
// Package e2e contains end-to-end tests for LDAP authentication.
//
// Requires a running GLAuth Docker container:
//
//	cd deploy/docker/glauth && docker compose up -d
//
// Run with:
//
//	cd tests/e2e && go test -v -tags=ldap -count=1 -timeout 120s -run TestE2E_LDAP .
package e2e
...
// testUser represents a known test user in the GLAuth config.
type testUser struct {
	username string
	password string
	wantRole string // expected Flowgent role after mapping
	domain   string
}
...
// ── Test: Full LDAP auth flow via provider ───────────────────────
...
func TestE2E_LDAP_Authenticate_Success(t *testing.T) {
	...
	for _, tu := range testUsers {
		t.Run(tu.username, func(t *testing.T) {
			// Simulate an HTTP login request
			...
			t.Logf("user=%s role=%s token=%s...", tu.username, user["role"], accessToken[:20])
		})
	}
}
...
func TestE2E_LDAP_JWTTokenValidation(t *testing.T) {
	...
	// Now validate the token by calling a protected endpoint
	// Create a test handler behind the auth middleware
	...
	if w2.Code != http.StatusOK {
		...
		t.Logf("JWT validation OK: %s", w2.Body.String())
	}
...
// ── Test: Direct LDAP search across domains ──────────────────────
...
// ── Test: Role mapping from AD groups ────────────────────────────
...
func TestE2E_LDAP_RoleMapping(t *testing.T) {
	...
	for _, tu := range testUsers {
		t.Run(tu.username, func(t *testing.T) {
			// We test the role mapping by doing a full login
			// The GLAuth server must be running
			if !ldapAvailable(t) {
				return
			}
			...
			if role != tu.wantRole {
				...
				t.Logf("user=%s role=%q OK", tu.username, role)
			}
		})
	}
}
...
func TestE2E_LDAP_ServerConnectivity(t *testing.T) {
	...
	// Anonymous bind should work for basic checks
	err = conn.Bind("", "")
	if err != nil {
		// Some LDAP servers require auth — that's OK, just check connectivity
	...
	for _, tu := range testUsers {
		...
		if len(result.Entries) == 0 {
			...
			t.Logf("user %s found: DN=%s mail=%v", tu.username, result.Entries[0].DN, result.Entries[0].Attributes["mail"])
		}
	}
}
