...
Orchestrates:
  ...
  2. Isolated Flowgent PostgreSQL schema reset
  ...
  5. Import e2e/config YAMLs into Flowgent DB (console import)
...
Usage:
  ...
  python3 runner.py --ui-rounds 5               # consecutive browser-driven rounds
  python3 runner.py --access                    # keep manual-access tunnels open
  python3 runner.py --clean-after-run            # opt in to resource teardown
...
def _restore_deployment_auth_token(namespace):
    """Restore the test control-plane token for a separate verify-only process."""
    if os.environ.get("FLOWGENT_E2E_AUTH_TOKEN"):
        return
    secret_name = f"{config.RESOURCE_PREFIX}-authorization"
    result = _sp.run(
        [
    ...
    if result.returncode != 0 or not result.stdout:
        return
    try:
        os.environ["FLOWGENT_E2E_AUTH_TOKEN"] = base64.b64decode(result.stdout).decode()
...
class _PortForwards:
    ...
    def __init__(self, namespace, release):
        self.namespace = namespace
        self.release = release
        self.processes = {}
        self.specs = (
            (f"{release}-apiserver", (f"{config.LOCAL_API_PORT}:9999",)),
            (f"{release}-a2a", (f"{config.LOCAL_A2A_PORT}:9992",)),
            (f"{release}-emqx", (f"{config.LOCAL_MQTT_PORT}:1883", f"{config.LOCAL_EMQX_DASHBOARD_PORT}:18083")),
    ...
    def _cleanup_stale_forwards(self):
        pattern = f"kubectl port-forward -n {self.namespace} service/{self.release}-"
    ...
    def _service_probe_ok(self):
        if not self._reachable(config.LOCAL_MQTT_PORT):
            return False
        try:
            with urllib.request.urlopen(f"http://127.0.0.1:{config.LOCAL_API_PORT}/_/healthz", timeout=1) as resp:
                if resp.status != 200:
                    return False
        except Exception:
            return False
        try:
            with urllib.request.urlopen(f"http://127.0.0.1:{config.LOCAL_A2A_PORT}/_/healthz", timeout=1) as resp:
                if resp.status != 200:
                    return False
        except Exception:
            return False
        try:
            with urllib.request.urlopen(f"http://127.0.0.1:{config.LOCAL_JAEGER_PORT}/api/services", timeout=1) as resp:
                if resp.status != 200:
                    return False
        except Exception:
            return False
    ...
    def ensure(self):
        for service, ports in self.specs:
...
SCENARIOS = {
    ...
    "14": ("AuthGuard — LDAP Federation + GitHub OAuth + Envoy Policy",    "verifier.s14_authguard_verifier"),
...
def _step_reset_flowgent_db():
    ...
    if not re.fullmatch(r"[a-z_][a-z0-9_]*", schema):
        print(f"  ERROR: invalid isolated PG schema name: {schema!r}")
        return False
    sql = (
        f'DROP SCHEMA IF EXISTS "{schema}" CASCADE; '
        f'CREATE SCHEMA "{schema}" AUTHORIZATION "{pg_user}";'
    )
    rc, _ = run_cmd([
...
def _step_build():
    ...
    print("\n-- Step: Build flowgent-ui image --")
    rc, _ = run_cmd(["make", "-C", PROJECT_ROOT, "build:image:ui"], timeout=1200, env=env)
    if rc != 0:
        print("  WARN: UI image build failed, retrying with HTTPS_PROXY=http://127.0.0.1:8800")
        rc, _ = run_cmd(["make", "-C", PROJECT_ROOT, "build:image:ui"], timeout=1200, env=env)
    if rc != 0:
        print("  ERROR: UI Docker image build failed")
        return False
...
def run_verification(args):
    ...
    forwards = _PortForwards(args.namespace, args.release).start()
    try:
...
def main():
    ...
    parser.add_argument(
        "--clean-after-run", action="store_true", default=False,
        help="Uninstall this isolated E2E deployment after success or failure (default: retain)",
    )
    parser.add_argument("--access", action="store_true", help="Keep manual-access tunnels open")
    parser.add_argument("--ui-rounds", type=int, help="Run consecutive browser-driven E2E rounds (minimum: 5)")
    parser.add_argument("--skip-first-build", action="store_true", help="Reuse images in the first UI round")
    ...
    parser.add_argument("--pg", help=f"PG DSN (default: {config.pg_dsn().replace('%', '%%')})")
    ...
    if args.access:
        from common.access import run as run_access
        run_access()
        return

    # Flow manifests use this value in sandbox network allowlists.  Initialize
    # it in verify-only mode too; deployment initialization alone is process-
    # local and must not be relied on by later runner invocations.
    from deploy.s11_flowgent_deployer import configure_runtime_proxy_env
    configure_runtime_proxy_env()

    exit_code = 1
    cleanup_requested = args.clean_after_run
    try:
        if args.ui_rounds is not None:
            from common.ui_e2e import run as run_ui_e2e
            exit_code = run_ui_e2e(args.ui_rounds, args.skip_first_build)
            return exit_code
        ...
        os.environ["FLOWGENT__STORAGE__POSTGRES__DSN"] = config.pg_dsn()
        os.environ["FLOWGENT__STORAGE__POSTGRES__SCHEMA"] = config.PG_SCHEMA
        if args.skip_deploy:
            _restore_deployment_auth_token(args.namespace)
        ...
        if all_passed and not cleanup_requested:
            print("\n  Helm deployment retained for manual experience:")
            print(f"    release={args.release} namespace={args.namespace}")
            print("    make e2e-security-fixer-access")
        return exit_code
    finally:
        if cleanup_requested:
            from deploy.s11_flowgent_deployer import cleanup_after_run
            if not cleanup_after_run(args.release, args.namespace):
                raise RuntimeError("isolated E2E cleanup did not complete")
