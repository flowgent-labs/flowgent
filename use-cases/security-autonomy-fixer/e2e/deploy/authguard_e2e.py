...
from common.images import (
    AUTHGUARD_IMAGE,
    AUTHGUARD_WEB_IMAGE,
    import_authguard_image_to_k3s,
    import_authguard_web_image_to_k3s,
)
...
SUPPORT_SCRIPT = Path(__file__).resolve().parents[1] / "support" / "mock_github.py"
...
def _ensure_secret(namespace: str) -> tuple[str, str]:
    ...
    if required.issubset(_existing_secret_keys(namespace)):
        print(f"  Reusing isolated AuthGuard Secret: {namespace}/{AUTH_SECRET}")
        return AUTH_SECRET, ""
    private_key, jwks = _session_key_and_jwks()
...
def _ensure_support(namespace: str, bind_password: str) -> None:
    if not bind_password:
        bind_password = _secret_value(namespace, "AUTHGUARD_LDAP_BIND_PASSWORD")
    ...
    _apply({
        ...
        "type": "Opaque",
        "stringData": {"ldap-config.cfg": _ldap_config(bind_password)},
    })
    _apply({
        "apiVersion": "v1",
        "kind": "ConfigMap",
        "metadata": {"name": f"{MOCK_GITHUB_NAME}-script", "namespace": namespace, "labels": labels},
        "data": {"mock_github.py": SUPPORT_SCRIPT.read_text(encoding="utf-8")},
    })
    for name, image, port, volume in (
        (LDAP_NAME, LDAP_IMAGE, 389, "ldap"),
        (MOCK_GITHUB_NAME, "localhost/flowgent-core:latest", 8080, "github"),
    ):
        mounts = []
        volumes = []
        command = None
        if volume == "ldap":
            ...
            command = ["python3", "/opt/e2e/mock_github.py"]
            mounts = [{"name": "script", "mountPath": "/opt/e2e/mock_github.py", "subPath": "mock_github.py", "readOnly": True}]
...
def runtime_config(namespace: str, pg_host: str, pg_port: str, pg_user: str) -> str:
    mock_url = f"http://{MOCK_GITHUB_NAME}.{namespace}.svc.cluster.local:8080"
    ldap_url = f"ldap://{LDAP_NAME}.{namespace}.svc.cluster.local:389"
    postgres_url = (
    ...
    document = {
        ...
        "cache": {
            ...
            "redis": {
                "nodes": [f"redis://{AUTHGUARD_REDIS_NAME}.{namespace}.svc.cluster.local:6379"],
                "username": "default",
...
def prepare(namespace: str, pg_container: str, pg_host: str, pg_port: str, pg_user: str, pg_password: str) -> dict:
    ...
    if not import_authguard_image_to_k3s():
        raise RuntimeError("isolated AuthGuard image is unavailable")
    if not import_authguard_web_image_to_k3s():
        raise RuntimeError("isolated AuthGuard Web image is unavailable")
    secret_name, bind_password = _ensure_secret(namespace)
    _reset_authguard_schema(pg_container, pg_user, pg_password)
    _ensure_support(namespace, bind_password)
    jwks = _secret_value(namespace, "AUTHGUARD_AUTHN_SESSION_JWKS")
    return {
        "authguard-middleware": {
            ...
            "authguard": {
                ...
                "authguard-config": runtime_config(namespace, pg_host, pg_port, pg_user),
            },
        },
    }
...
def _social_login(namespace: str) -> tuple[str, str]:
    ...
    with _forward(namespace, authn_service, 8082) as authn_port, _forward(namespace, MOCK_GITHUB_NAME, 8080) as mock_port:
...
def _policy(revision: int, github_principal_id: str) -> dict:
    ...
    bindings = [
        {"id": "github-e2e-platform-admin", "principal_id": github_principal_id, "role_id": "flowgent-platform-security-admin", "effect": "ALLOW", "resource_urn": tenant_scope, "conditions": {}},
...
def bootstrap_and_verify(namespace: str) -> None:
    print("\n-- Verifying LDAP federation, GitHub AuthN, and Envoy/AuthGuard authorization --")
    _wait_authguard_ready(namespace)
    token, github_principal_id = _social_login(namespace)
    api_token = _secret_value(namespace, "AUTHGUARD__AUTHZ__API_TOKEN")
    ...
    with _forward(namespace, authz_service, 9091) as port:
        for text_value, kind, principal_id in materializations:
            status, _, result = _api(port, "/api/v1/principal-discovery/search", api_token, "POST", {"text": text_value, "kinds": [kind], "provider_ids": ["e2e-flowgent-corporate-ldap"], "per_provider_limit": 20, "cursors": {}})
            ...
            status, _, _ = _api(port, "/api/v1/principal-discovery/materialize", api_token, "POST", {"principal_id": principal_id, "reference": candidates[0]["reference"]})
            if status != 201:
                raise RuntimeError(f"LDAP materialization failed for {text_value}: HTTP {status}")
        status, headers, current = _api(port, "/api/v1/policy", api_token)
        if status != 200:
            raise RuntimeError(f"read AuthGuard policy failed: HTTP {status}")
        revision = int(current.get("revision", 1))
        status, _, persisted = _api(port, "/api/v1/policy", api_token, "PUT", _policy(revision, github_principal_id), {"If-Match": str(revision)})
    ...
    with _forward(namespace, service.strip(), 80) as gateway_port:
        status, _, metadata = _api(
            gateway_port,
    ...
    print(
        ...
        "discovery/materialization + GitHub OAuth + gateway allow/deny"
    )
