...
class OtelVerifier(BaseVerifier):
    ...
    def _resolve_env_vars(obj):
        ...
        if isinstance(obj, dict):
            return {k: OtelVerifier._resolve_env_vars(v) for k, v in obj.items()}
        if isinstance(obj, list):
            return [OtelVerifier._resolve_env_vars(v) for v in obj]
        return obj
        ...
        for mode in ("github", "sonarqube"):
            if _USE_REAL_MCP:
                with open(os.path.join(_CONFIG_ROOT, "mcps", f"{mode}.yaml")) as f:
                    mcp_def = OtelVerifier._unwrap_k8s(yaml.safe_load(f))
            else:
            ...
            if existing.status_code == 200:
                ...
                OtelVerifier._get_or_post(f"/api/v1/{NAMESPACE}/mcp/{mode}", f"/api/v1/{NAMESPACE}/mcp", mcp_def, "mcp")
    ...
    def query_jaeger_trace(run_id: str, jaeger_base_url: str,
        ...
        **(MANDATORY)** Query Jaeger for traces matching ``run.id``.
        ...
        while time.time() < deadline:
            try:
                resp = OtelVerifier._jaeger_get(url)
                ...
                for trace in traces:
                    for span in trace.get("spans", []):
                        tags = OtelVerifier._span_tags(span)
    ...
    def validate_span_attributes(all_spans: list, sample_count: int = 12):
        ...
        """
        tagged = [s for s in all_spans if OtelVerifier._span_tags(s).get("flowgent.node_id")]
        ...
        for s in sample:
            tags = OtelVerifier._span_tags(s)
    ...
    def ensure_security_fixer_flow_exists():
        ...
        with open(_FLOW_YAML_PATH) as f:
            flow_def = OtelVerifier._resolve_env_vars(OtelVerifier._unwrap_k8s(yaml.safe_load(f)))
        flow_def.pop("name", None)
        flow_def.pop("triggers", None)
        flow_def["kind"] = "flow"
        path = f"{API_BASE}/api/v1/{NAMESPACE}/flows/security-autonomy-fixer"
        existing = FLOWGENT_SESSION.get(path, timeout=10)
        if existing.status_code == 200:
            resp = FLOWGENT_SESSION.put(path, json=flow_def, timeout=10)
        else:
            resp = FLOWGENT_SESSION.post(
                f"{API_BASE}/api/v1/{NAMESPACE}/flows", json=flow_def, timeout=10,
            )
    ...
    def _runtime_otel_evidence(namespace: str, selector: str, component: str) -> str:
        ...
        """
        pod_name = OtelVerifier._run_text(
            [
        ...
        env_endpoint = ""
        if pod_name:
            config_text = OtelVerifier._run_text(
                ["kubectl", "exec", "-n", namespace, pod_name, "--", "cat", "/etc/flowgent/flowgent.yaml"],
                timeout=10,
            )
            env_endpoint = OtelVerifier._run_text(
    ...
    def get_jm_pod_logs() -> str:
        ...
        try:
            ...
            if "OTEL tracing enabled" in logs:
                return logs
            evidence = OtelVerifier._runtime_otel_evidence(
                JM_NAMESPACE,
    ...
    def _verify_scenario():
        ...
        OtelVerifier.seed_agents_and_mcps()
        OtelVerifier.ensure_security_fixer_flow_exists()
        run_id = OtelVerifier.trigger_security_fixer()
        status = OtelVerifier.wait_for_completion(run_id, timeout=600)
        if status != "COMPLETED":
        ...
        jm_logs = OtelVerifier.get_jm_pod_logs()
        apiserver_logs = OtelVerifier.get_apiserver_logs()

        otel_checks = OtelVerifier.verify_otel_from_logs(jm_logs, apiserver_logs)

        # ── Phase B: MANDATORY Jaeger trace query ──
        print(f"\n  -> [MANDATORY] Querying Jaeger for traces (run.id={run_id})...")
        if not OtelVerifier._check_jaeger_health(JAEGER_API):
            raise AssertionError(
                f"MANDATORY: Jaeger Query API at {JAEGER_API} is not reachable. "
                "Ensure Jaeger all-in-one is running and accessible."
            )

        traces, all_spans = OtelVerifier.query_jaeger_trace(run_id, JAEGER_API, timeout_sec=90)

        # ── Phase C: MANDATORY span coverage validation ──
        print("\n  -> [MANDATORY] Validating per-node span coverage...")
        node_counts = OtelVerifier.validate_span_coverage(all_spans, fail_on_core_missing=True)
        ...
        OtelVerifier.validate_span_attributes(all_spans)
    ...
    def _verify_docker_scenario():
        ...
        traces, spans = OtelVerifier.query_jaeger_trace(run_id, JAEGER_API, timeout_sec=90)
        node_counts = OtelVerifier.validate_span_coverage(spans, fail_on_core_missing=True)
        ...
        OtelVerifier.validate_span_attributes(spans)
    ...
    def run(self) -> VerificationResult:
        ...
        return self.execute(lambda: self.step("verify runtime OTEL export and Jaeger span coverage", operation))

    @staticmethod
    def _verify_kubernetes() -> None:
        OtelVerifier._verify_scenario()

    @staticmethod
    def _verify_docker() -> None:
        OtelVerifier._verify_docker_scenario()
