...
def _pg_query(query):
    ...
    result = subprocess.run(
        ["sudo", "docker", "exec", "sigbot_e2e_164364_postgres", "sh", "-c",
         f"PGPASSWORD=test PGOPTIONS='-c search_path={config.PG_SCHEMA}' "
         f"psql -U test -d flowgent -XAt -c '{escaped}'"],
...
def run():
    ...
    # ── L3.1: Agents ─────────────────────────────────────────
    result = _pg_query(f"SELECT COUNT(*) FROM llm_agent WHERE namespace_id='{NAMESPACE}' AND del_flag=false;")
    if result.returncode == 0:
        ...
        if count >= MIN_AGENTS:
            ...
            result2 = _pg_query(f"SELECT name FROM llm_agent WHERE namespace_id='{NAMESPACE}' AND del_flag=false ORDER BY name;")
    ...
    # ── L3.3: MCPs ───────────────────────────────────────────
    result = _pg_query(f"SELECT COUNT(*) FROM llm_mcp WHERE namespace_id='{NAMESPACE}' AND del_flag=false;")
    if result.returncode == 0:
        count = int(result.stdout.strip() or "0")
        ...
        if count >= MIN_MCPS:
            ...
            result2 = _pg_query(f"SELECT name FROM llm_mcp WHERE namespace_id='{NAMESPACE}' AND del_flag=false ORDER BY name;")
            if result2.returncode == 0 and result2.stdout.strip():
                for line in result2.stdout.strip().splitlines():
    ...
    # ── L3.4: LLM Providers ──────────────────────────────────
    result = _pg_query(f"SELECT COUNT(*) FROM llm_providers WHERE namespace_id='{NAMESPACE}' AND del_flag=false;")
    if result.returncode == 0:
        count = int(result.stdout.strip() or "0")
    ...
    result = _pg_query(f"SELECT COUNT(*) FROM nfy_channel WHERE namespace_id='{NAMESPACE}' AND del_flag=false;")
    if result.returncode == 0:
        count = int(result.stdout.strip() or "0")
        ...
        if count >= MIN_NOTIFIERS:
            ...
            result2 = _pg_query(f"SELECT name FROM nfy_channel WHERE namespace_id='{NAMESPACE}' AND del_flag=false ORDER BY name;")
            if result2.returncode == 0 and result2.stdout.strip():
                for line in result2.stdout.strip().splitlines():
