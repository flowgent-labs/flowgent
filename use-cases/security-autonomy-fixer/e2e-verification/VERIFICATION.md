...
## 1.3 Two-Step Workflow

Every time you tear down and re-run this use case, execute the two entry points in order:

```bash
# Step 1 — From-zero setup (SonarQube PG reset + binary build + config import)
cd use-cases/security-autonomy-fixer
python3 runner.py

# Step 2 — Verification (scenario-based checks against the live system)
cd use-cases/security-autonomy-fixer/e2e-verification
python3 verification.py
```

| Script | Purpose |
|--------|---------|
| `runner.py` | Master entry point: resets SonarQube PG, builds the binary, imports all config YAMLs via `console import`. Run this first. |
| `verification.py` | Verification entry point: runs all scenario checks (01–13) against the live system. Run this second. |

### Step 2 — Verification Quick Start
...
python3 verification.py -s 01  # Infrastructure verification
python3 verification.py -s 02  # API Server CRUD
python3 verification.py -s 03  # A2A protocol validation
python3 verification.py -s 04  # Controller -> JM pod lifecycle
python3 verification.py -s 05  # Messager topics + Sandbox chain
python3 verification.py -s 06  # Notifier connectivity
python3 verification.py -s 07  # Wallet key mgmt + MQTT signing
python3 verification.py -s 08  # OTEL / Jaeger tracing
python3 verification.py -s 09  # E2E security fixer capstone
python3 verification.py -s 10  # PR commit verification
python3 verification.py -s 11  # Knowledge RAG
python3 verification.py -s 12  # Volume workspace
python3 verification.py -s 13  # Console import

# List all scenarios
python3 verification.py -l
python3 verification.py --api http://10.0.0.1:9999 --pg postgres://u:p@h/db
```

**All verification scenario execution MUST go through `verification.py`.** Do not
manually execute individual `scenarios/*.py` files.
...
### 2.2.2 Automatic Seeding (via runner.py console import)

The use-case `runner.py` step 3 imports all config YAMLs into the Flowgent
...
curl -s -X POST http://localhost:9999/api/v1/default/agents \
  -H "Content-Type: application/json" \
  -d @use-cases/security-autonomy-fixer/config/agents/issue-detector.yaml
```
...
- **Flow Definition**: `use-cases/security-autonomy-fixer/config/flows/security-autonomy-fixer.yaml`
- **Agent Definitions**: `use-cases/security-autonomy-fixer/config/agents/*.yaml`
- **MCP Servers**: `use-cases/security-autonomy-fixer/config/mcps/`
- **Use-Case Runner**: `use-cases/security-autonomy-fixer/runner.py` (from-zero setup)
- **Verification Runner**: `use-cases/security-autonomy-fixer/e2e-verification/verification.py` (scenario checks)
- **Scenario Scripts**: `use-cases/security-autonomy-fixer/e2e-verification/scenarios/*.py`
