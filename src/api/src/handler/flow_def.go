...
func (h *FlowDefHandler) notifyWatchers() {
	h.mu.Lock(); h.watchVersion++; chs := h.watchChs; h.watchChs = nil; h.mu.Unlock()
	for _, ch := range chs { close(ch) }
}

func (h *FlowDefHandler) Watch(w http.ResponseWriter, r *http.Request) {
	since, _ := strconv.ParseInt(r.URL.Query().Get("since"), 10, 64)
	h.mu.RLock(); cur := h.watchVersion; h.mu.RUnlock()
	if cur > since { h.List(w, r); return }
	ch := make(chan struct{})
	h.mu.Lock(); h.watchChs = append(h.watchChs, ch); h.mu.Unlock()
	select {
	case <-ch: h.List(w, r)
...
func (h *FlowDefHandler) Trigger(w http.ResponseWriter, r *http.Request) {
	var req struct { AgentFlowID string `json:"agentflow_id"`; Vars map[string]any `json:"vars"`; Trigger model.TriggerInfo `json:"trigger"` }
...
func (h *FlowDefHandler) TriggerByID(w http.ResponseWriter, r *http.Request) {
	var req struct { Vars map[string]any `json:"vars"`; Trigger model.TriggerInfo `json:"trigger"` }
