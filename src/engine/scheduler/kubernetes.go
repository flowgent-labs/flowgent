...
func NewKubernetesResourceManager(cfg *ResourceManagerConfig) (*KubernetesResourceManager, error) {
	if cfg.SlotsPerTM <= 0 { cfg.SlotsPerTM = 4 }
	if cfg.MinTMs <= 0     { cfg.MinTMs = 2 }
	if cfg.MaxTMs <= 0     { cfg.MaxTMs = 10 }
	if cfg.IdleTimeout <= 0 { cfg.IdleTimeout = 5 * time.Minute }
	if cfg.PlanTimeout <= 0 { cfg.PlanTimeout = 5 * time.Minute }
	if cfg.K8sNamespace == "" { cfg.K8sNamespace = "default" }
	if cfg.K8sDeploymentName == "" { cfg.K8sDeploymentName = "flowgent-taskmanager" }
...
func (s *KubernetesResourceManager) reconcile(ctx context.Context) {
	...
	if pending > int64(currentSlots) {
		...
		if neededTMs > s.maxTMs { neededTMs = s.maxTMs }
	...
	if idleDuration > s.idleTimeout && currentTMs > s.minTMs {
		...
		if targetTMs < s.minTMs { targetTMs = s.minTMs }
