...
func (s *PostgresStore) CreateAgentFlowRun(ctx context.Context, run *model.AgentFlowRun) error {
	...
	os.WriteFile("/tmp/pgx_debug.log", []byte(fmt.Sprintf("PGX_EXEC id=%s flow=%s\n", run.ID, run.AgentFlowID)), 0644)
	...
	os.WriteFile("/tmp/pgx_debug.log", []byte(fmt.Sprintf("PGX_EXEC rows=%d err=%v\n", r.RowsAffected(), err)), 0644)
	return err
}
