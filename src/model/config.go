...
type AppConfig struct {
	...
	Flows    []AgentFlowSpec           `json:"flows" yaml:"flows"`
	SubFlows map[string]AgentFlowSpec  `json:"sub_flows,omitempty" yaml:"sub_flows,omitempty"`
}
...
// then sub-flows.
func (c *AppConfig) GetFlow(id string) *AgentFlowSpec {
	for i := range c.Flows {
		if c.Flows[i].ID == id {
			return &c.Flows[i]
		}
