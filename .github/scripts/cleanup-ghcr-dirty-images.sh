...
# Delete only GHCR image versions tagged by commits from one merged PR.
# Cleanup is best-effort: artifact publication must not fail when the package
# API is unavailable or a manifest also carries an unrelated tag.
set -uo pipefail
...
# Timeline SHAs retain heads removed by force-pushes.
